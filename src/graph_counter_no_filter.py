...
filters by a chosen induced subgraph if needed, and writes Macaulay2
input for computing toric ideals and minimal generators.

This version is stripped down for the project:
- plotting helpers from the larger script were removed
...
class ContainingRecord(GraphRecord):
    """Graph record together with a witness for the induced subgraph."""
...
def graph_to_canonical_form(n: int, edges: Sequence[Edge]) -> CanonicalForm:
    ...
    # For n >= 1, best is always set, including the empty graph.
...
def generate_macaulay2_code(
    ...
    with path.open("w", encoding="utf-8") as f:
        ...
        for idx, rec in enumerate(graphs, start=1):
            ...
            # Keep the same behaviour as before: only introduce vertex variables
            # for vertices that actually occur in the edge list.
            ...
            for j, (u, v) in enumerate(edges, start=1):
                ...
                # Keep the vertex labels in the comment 1-indexed as before.
...
def make_induced_highlighter(subgraph_edges: Sequence[Edge]) -> Callable[[GraphRecord], Iterable[Edge]]:
    ...
    For ContainingRecord objects, this uses the stored mapping to recover the
    corresponding edges in the larger graph.

    For plain GraphRecord objects, it returns an empty list.
    """
...
def main() -> None:
    ...
    - write a Macaulay2 script for all of them;
    ...
    # Write Macaulay2 input for all graphs.
    m2_file = Path(paths["m2"]) / f"toric_analysis_all_n{n}.m2"
    out_file = generate_macaulay2_code(graphs, n=n, filename=m2_file.name, out_dir=str(m2_file.parent))
    print(f"Wrote Macaulay2 input: {out_file}")

    # Run Macaulay2 if requested.
    if run_m2:
        log_file = Path(paths["logs"]) / f"toric_analysis_all_n{n}.txt"
